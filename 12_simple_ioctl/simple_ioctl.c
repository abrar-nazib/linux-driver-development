...
static char* mod_name = "simple_ioctl";
#define DEVICE_NAME "simple_ioctl" // -> Creates /dev/hello_cdev
#define CLASS_NAME "simple_ioctl"
...
static long int my_ioctl(struct file* f, unsigned int cmd, unsigned long args){
    pr_info("%s: Ioctl called with cmd: %d and args %p\n", mod_name, cmd, (void*) args);
    return 0;
}
